...
public class CacheSim {
	...
	public static void main(String[] args) {
		...
		int CACHE_MAX_BLOCKS = input*1024*1024;
                HashMap<Block,Integer> hmap = new HashMap<Block, Integer>();
                Block[] b2 = new Block[1000000000];
		...
		bfs.ShortestPath(G, bfs, source, destination);
		*/ 
		...
		while(inLogFile.hasNextLine()){
			...
			G.cache.get(b.src).enqueue(b);
				...
				Queue<Block> q1 = G.cache.get(i);
                                Block block = q1.dequeue();
                                b2[i] = block;}
		while(true){
				...
				if (!q.isEmpty()) {
					...
					switch(policyName){
						...
						case 5:
							...
							break;
						case 6:
							G.returnVertex(i).getOPT().accessCache(block,b2);
							break;
						case 7:
